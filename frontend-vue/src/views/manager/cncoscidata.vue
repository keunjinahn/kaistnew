    ...
    <CCard>
      <v-app class="sr-card-app-title">
        <CCardHeader>
          ...
          <strong> KAIST 공작기계 진동 데이터 </strong>
        </CCardHeader>
        <CCardBody>
          <CNavbar
              expandable="md"
        ...
        <v-dialog v-model="sensorDetail.show" persistent max-width="840px" >
          <v-card>
            <div class="sensor-detail-container">
              ...
              <cncosci-detail-plc @callBackEvent="onEmitEvent" :cncosci_data="sensorDetail.data" />
            </div>
          </v-card>
        </v-dialog>
        <v-dialog v-model="sensorGraph.show" persistent max-width="1200px">
          <v-card >
            <div class="sensor-detail-container-2">
              <CCardHeader>
                <CIcon name="cil-justify-center"/>
                <strong> KAIST IoT 데이터 그래프(공작기계 진동) </strong>
              </CCardHeader>
...
<script>
import CncosciDetailPlc from '@/components/CncosciDetailPlc'
...
export default {
  components: {CncosciDetailPlc,moment},
  methods: {
    async query() {
    ...
    async get_cncosci_data(id){
      ...
      this.cncosci_data_cate = data.objects.map(v => v.created_date.slice(-8))
      this.cncosci_data_objs = data.objects.map((v,index) => ({
        x: v.index,
  ...
  computed: {
    ...
    chart6Option () {
      ...
      obj.xAxis[0].categories = this.cncosci_data_cate
      obj.series[0].name = '진동데이터,CH0'
      return obj
    },
  },
  watch: {
